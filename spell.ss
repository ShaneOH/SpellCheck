...
(define gen-hash-hybrid-method
  ...
  ;;(2*division(k) + 3 * multiplication(k)) mod size
  	(lambda (k)
...
(define gen-checker
  (lambda (hashfunctionlist dict)
	(let ((bitvector 
		(flatten (map (lambda (a) (append (map (lambda (b) (b a)) hashfunctionlist))) dict)
		)))
		(lambda (k)
			(let ((w 
				(flatten (map (lambda (a) (append (map (lambda (b) (b a)) hashfunctionlist))) (list k)))))
				(if (andmap (lambda (current) (member current bitvector)) w) #t #f))
))))
